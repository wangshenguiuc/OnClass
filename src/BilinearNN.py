...
class BilinearNN:
	...
	def train(self, train_X, train_Y, Y_emb, nseen,  nhidden=[100], max_iter=500, minibatch_size=32, lr = 0.0001, l2=0.01):
		...
		self.__build_loss()
		self.optimize()
		train_Y_pred = self.predict_score(sess, self.train_X, self.train_Y_emb)
		return train_Y_pred

	def predict(self, test_X, reset_model=True):
		test_X = np.array(test_X, dtype=np.float64)
		test_Y_pred = self.predict_score(sess, test_X, self.Y_emb)
		if reset_model:
	...
	def predict_score(self, sess, X, Y_emb):
		p = sess.run(self.p, feed_dict={self.mini_train_X: X, self.mini_train_Y_emb: Y_emb})
		score = softmax(p, axis=1)
		return score

	def predict_prob(self, sess, X, Y, Y_emb):
		label = Y
		p = sess.run(self.p, feed_dict={self.mini_train_X: X, self.mini_train_Y_emb: Y_emb})
		p = softmax(p, axis=1)
	...
	def optimize(self):
		...
		for epoch in range(self.max_iter):
			...
			for minibatch in minibatches:
				...
				minibatch_cost, p, label, entropy,  _ = sess.run([self.loss, self.p, self.label, self.entropy, train_op], feed_dict={self.mini_train_X: minibatch_X, self.mini_train_Y: minibatch_Y, self.mini_train_Y_emb: self.train_Y_emb})
				epoch_cost += minibatch_cost
			if (epoch+1) % 1 == 0:
				train_acc,_,train_auroc = self.predict_prob(sess, self.train_X, self.train_Y, self.train_Y_emb)
