...
dnames = ['microcebusBernard','microcebusStumpy','microcebusMartine','muris_droplet']#'microcebusAntoine',
dnames = ['microcebusAntoine']
		...
		for unseen_ratio in unseen_ratios:
			...
			for i in unseen_l:
				if len(np.unique(Y_truth_bin_mat[:,i]))==2:
					...
					#class_auprc_macro[i] = average_precision_score(Y_truth_bin_mat[:,i], pred_Y_all[:,i])
					aucs.append(class_auc_macro[i])
					sps.append(np.min(sp[:nseen,i]))
			print (stats.spearmanr(sps, aucs), len(aucs),np.unique(sps))

	np.save(result_dir+dname+'_sps.npy', sps)
	np.save(result_dir+dname+'_aucs.npy', aucs)
