...
for dnamei, dname1 in enumerate(dnames):
	...
	nmarkers = []
	fout = open(supple_dir + dname1+'_OnClass_referred_cl.txt', 'w')
	fin = open('/oak/stanford/groups/rbaltman/swang91/Sheng_repo/software/OnClass_data/' + 'cell_ontology/cl.obo')
	for line in fin:
	...
	for line in fin:
		...
		if line.startswith('name: '):
			if name2co[line.strip().split('name: ')[1]] in l2i1:
				cid = l2i1[name2co[line.strip().split('name: ')[1]]]
				our_marker = np.where(sub_cor[cid,:]==1)[0]#np.argsort(cor[l2i1[tp],:]*-1)[:topk]
				if len(our_marker) ==0:
					our_marker = np.argsort(sub_cor[cid,:]*-1)[:topk]
				cnum.append(len(our_marker))
				if len(our_marker)>=389 or len(our_marker)==1:
					print (len(our_marker), line.strip().split('name: ')[1])
				fout.write('OnClass-computed marker genes: ')
				for gi in our_marker:
					fout.write(i2g1[gi]+', ')
				fout.write('\n')
	fin.close()
	fout.close()
	print (np.median(cnum), np.mean(cnum), np.max(cnum), np.min(cnum))
