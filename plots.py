...
def plot_cell_type_freq_auprc(jitter_datas, datas, fig_dir, xlabels):
	plt.clf()
	...
	ax = sns.violinplot( data=datas,
	                    inner='box', color=".8",showmeans=True,
	                         showextrema=False)
	ax.set_ylabel('AUPRC (unseen)')
	ax.set_xlabel('Number of positive labels')
	#ax.violinplot(datas)
	ax.set_xticklabels(xlabels)
	#ax.set_yticklabels(np.arange(0, 1, 0.2))
	#ax.set_ylim([0., 1.0])
	ax.spines['right'].set_visible(False)
	ax.spines['top'].set_visible(False)
	plt.tight_layout()
...
def plot_expression_embedding_spearman(d, e, tis,fig_file):
	...
	xmax = 1.
	ymax = 1.
	plt.yticks(np.arange(ymin, ymax+0.01, 0.2))
	plt.xticks(np.arange(xmin, ymax+0.01, 0.2))
...
def plot_26dataset_bar_auprc(ctypes, aucs, is_seen, output_file):
...
def plot_nlp_effect_bar(mean, error, group_l, method_l, output_file, ylabel='AUPRC (unseen)'):
	...
	plt.legend(loc='upper left',bbox_to_anchor=(0.5, 1.1), frameon=False, ncol=1, fontsize=4)
	...
	ax.set_ylim([0.1, 0.3])
	step_size = 0.05
	ax.yaxis.set_ticks(np.arange(0.1, max_y + 0.1-0.0000001, step_size))
	#ax.legend(method_l)
	fig.tight_layout()
	plt.savefig(output_file)


def plot_nlp_correlated_expression(tissues, pears, output_file):
	pears = np.array(pears)
	tissues = np.array(tissues)
	ind = np.argsort(pears*-1)
	pears = pears[ind]
	print (pears)
	tissues = tissues[ind]
	print (tissues)
	...
	width = 0.85
	bars = plt.bar(y_pos, pears, align='edge', alpha=1, width=width, color='g')
	plt.xticks(y_pos+width/2, tissues, rotation=90)
	ax.set_ylabel('Correlation')
	ax.spines['right'].set_visible(False)
	ax.spines['top'].set_visible(False)
	plt.ylim([-0.3, 0.7])
	plt.yticks([-0.3, -0.1, 0.1, 0.3, 0.5, 0.7])
	plt.tight_layout()
	plt.savefig(output_file)
...
def plot_auprc_curve(labels, pred, title, fig_file):
	#fpr,tpr = roc_curve(labels, pred)[:2]
	fpr, tpr, _ = precision_recall_curve(labels, pred)
	auc = average_precision_score(labels, pred)
	fig, ax = plt.subplots(figsize=(FIG_WIDTH,FIG_HEIGHT))
	lw = 1
	plt.plot(fpr, tpr, color='darkorange',
			 lw=lw, label='AUPRC = %0.2f'%auc)
	plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
	plt.xlim([0.0, 1.0])
	plt.ylim([0.0, 1.05])
