...
class OnClassModel:
	...
	def Train(self, train_feature, train_label, save_model = None, max_iter=15, minibatch_size = 128):
		"""
		...
		self.model.optimize(max_iter = max_iter, mini_batch_size = minibatch_size)
	...
	def Predict(self, test_feature, test_genes=None, use_normalize=False, refine = True, unseen_ratio = 0.1):
		"""
		...
		with torch.no_grad():
			if not self.mode:
				test_Y_pred_seen = softmax(self.model.forward(test_feature, training=False), axis=1)
	...
	def _batch_predict(self, X, num_batches, mapping=None):
		...
		for k in range(0, num_batches):
			...
			Y[k * batch_size : (k+1) * batch_size,:] = self.model.forward(X_array, training=False)
