...
from utils import pickle_these_objects, MapLabel2CL, evaluate, unpickle_from_file, read_ontology_file, read_data, run_scanorama_multiply_datasets, print_memory_usage
from config import ontology_data_dir, scrna_data_dir, model_dir, Run_scanorama_batch_correction, NHIDDEN, MAX_ITER, MEMORY_SAVING_MODE
...
print_memory_usage("BEFORE READING TEST DATA")
...
begin = datetime.now()
print("Starting to process train features at", begin)
OnClass_train_obj.ProcessTrainFeature( train_feature, train_label, train_genes, test_feature = test_feature, test_genes = test_genes, batch_correct=False)
print("\tprocessing train features took", datetime.now() - begin)

print_memory_usage("after processesing train features")
...
begin = datetime.now()
print("Starting to train model at", begin)
...
print("\tTraining took", datetime.now() - begin)

print_memory_usage("after training model")

print ('initialize test model. Load the model from $model_path...')
begin = datetime.now()
print("Starting to process test features at", begin)
...
#cor_test_feature = OnClass_train_obj.ProcessTestFeature(
...
print("\tProcessesing test features took", datetime.now() - begin)


# Prediction
begin = datetime.now()
print("Starting to predict at", begin)
pred_Y_seen, pred_Y_all, pred_label = OnClass_test_obj.Predict(
	cor_test_feature, test_genes = cor_test_genes, use_normalize=False)
print("\tPredicting took", datetime.now() - begin)

pred_label_str = [OnClass_test_obj.i2co[l] for l in pred_label]

print()
print("Takes", datetime.now() - begin_program, "to run OnClass")
print()
